...
unlink("offline4.php");
$autocomplete = "offline4.php"; 
 $myFile = fopen($autocomplete, "w");

fputs($myFile,  ' <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en" manifest="cache.manifest" >
<head>
...
<div class="container">
  ...
  fputs($myFile,  '<script> 
  var countries = ["ACCOUNT NUMBER",');?>
<?php   	
$x="select ACCOUNT,METERNUMBER,EMAIL,DATE2 FROM  $accountstable WHERE STATUS ='CONNECTED' AND ACCOUNT IN(SELECT ACCOUNT FROM $meterstable) ORDER BY ACCOUNT   ";
$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		 while ($y=@mysqli_fetch_array($x))
		{$details=$y['ACCOUNT']."||METER:".$y['METERNUMBER']."||PREV:".$y['EMAIL']."||DATE:".$y['DATE2'];fputs($myFile, '"'.$details.'",');	}
...
<br>
  ...
  fputs($myFile,  '<script> 
  var accounts = ["ACCOUNT NUMBER",');?>
<?php   	
$x="select ACCOUNT  FROM  $accountstable WHERE STATUS ='CONNECTED' AND ACCOUNT IN(SELECT ACCOUNT FROM $meterstable) ORDER BY ACCOUNT   ";
$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		 while ($y=@mysqli_fetch_array($x))
		{$details=$y['ACCOUNT'];fputs($myFile, '"'.$details.'",');	}
...
autocomplete(document.getElementById("account"),accounts);
  ...
  fputs($myFile,  '<script> 
  var meters = ["METER NUMBER",');?>
<?php   	
$x="select METERNUMBER  FROM  $accountstable WHERE STATUS ='CONNECTED' AND ACCOUNT IN(SELECT ACCOUNT FROM $meterstable)  ORDER BY METERNUMBER   ";
$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		 while ($y=@mysqli_fetch_array($x))
		{$details=$y['METERNUMBER'];fputs($myFile, '"'.$details.'",');	}
