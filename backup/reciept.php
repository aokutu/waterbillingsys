...
if(mysqli_num_rows($x)>0){}
 ...
 $x="SELECT CLIENT,ACCOUNT,ID,IFNULL(DEPOSIT,0) AS REFUNDDEPOSIT FROM $accountstable WHERE  $accountstable.account='".$_SESSION['account']."'";
			$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		
		 while ($y=@mysqli_fetch_array($x))
		{ 	$refunddeposit=$y['REFUNDDEPOSIT'];$client=$y['CLIENT'];$account=$y['ACCOUNT'];$recieptnumber=strtoupper(str_pad($y['ID'], 10, "0", STR_PAD_LEFT)); }}
				...
				<tr >
					<td colspan="2">
						<table>
							<tr>
								<td class="title">
									<h2 style="text-align:center;font-weight:bold;">REFUND DEPOSIT RECEIPT</h2>
								</td>
...
<?php print strtoupper(str_pad($recieptnumber, 10, "0", STR_PAD_LEFT)); ?>				</td>
	...
	print "<tr class='item'>";	
		print "<td  style='border: 1px solid black; border-collapse: collapse;text-align:center;'>REFUND DEPOSIT</td>";	
		print "<td  style='border: 1px solid black; border-collapse: collapse;text-align:center;' >".number_format($refunddeposit,2)."</td>";	
		print "</tr>";
