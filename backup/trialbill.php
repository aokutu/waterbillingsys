...
if(mysqli_num_rows($x)>0){}
  ...
  $x=" SELECT ID,ACCOUNT,METERNUMBER  FROM $nonwaterbills WHERE  ID ='".$_SESSION['id']."'  ORDER BY DATE,ID DESC LIMIT 1  ";
	...
	$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		...
		{ $reffnumber=$y['ID'];$identity=$y['ID'];$account=$y['ACCOUNT'];$meternumber=$y['METERNUMBER']; }}	
	...
	$x="SELECT * FROM $accountstable  WHERE  account='$account' ";
		...
		if(mysqli_num_rows($x)>0)
		{
		 ...
		 while ($y=@mysqli_fetch_array($x))
		{  $client=$y['client'];$account=$y['account']; $status=$y['status'];$size=$y['size']; $class=$y['class']; $lastreading=$y['email'];$location=$y['location'];  $contact=$y['contact'];$deposit=number_format($y['deposit'],2);}}
				...
				<tr >
					<td colspan="2">
						<table>
							<tr>
								<td class="title">
									<h2 style="text-align:center;font-weight:bold;">NON WATER BILL </h2>
								</td>
...
<?php print strtoupper(str_pad($recieptnumber, 10, "0", STR_PAD_LEFT)); ?>				</td>
	 ...
	 <?php print $recieptdate; ?></td></tr>
				...
				<tr class="heading" style="font-weight: bold;text-align:center;" >
					...
					<td  style='border: 1px solid black; border-collapse: collapse;text-align:center;'>Price</td>
				</tr>
						
				<?php
				
					$id=$_SESSION['id'];							
		$x="SELECT NAME,AMOUNT FROM $nonwaterbills WHERE  ID =$id ";
		$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		...
		print "<td  style='border: 1px solid black; border-collapse: collapse;text-align:center;' >".number_format($y['AMOUNT'],2)."</td>";	
		print "</tr>";	
		}}
...
<table>
					<tr>
				<td style="text-align:center;font-weight:bold;border: 1px solid black; border-collapse: collapse;"  >UNLESS PAYMENT OF THE TOTAL AMOUNT OUTSTANDING ON YOUR ACCOUNT IS RECEIVED ON OR BEFORE YOUR WATER SUPPLY SHALL BE DISCONNECTED</td>	
					</tr>	
<tr>
				<td style="text-align:center;font-weight:bold;border: 1px solid black; border-collapse: collapse;">PAY TO:K.C.B ACCOUNT NUMBER 1108110320<br>Equity Account Number 1270298313208<br>Co-op Account Number 01136561808400<br>M-pesa Pay Bill No.522237 <br>EQUITY PAYBILL 4040391<br>CO-OP PAY BILL 400222 Business Code 538199#A/c No.</td>	
					</tr>
<tr>
				<td style="text-align:center;font-weight:bold;border: 1px solid black; border-collapse: collapse;">Note:All Enquiries Customer Care No.0706634472</td>	
					</tr>					
