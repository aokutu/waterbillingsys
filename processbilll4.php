...
$processbill->bedcharges=$connect->real_escape_string(trim(addslashes(strtoupper($_SESSION['bedcharges']))));
...
$connect->query("INSERT INTO receiptrecords(RECEIPTNUMBER,PAYMODE,PAYMENTREFF,AMOUNT,DISCOUNT,TAX,TOTALCHARGES,DATE,CASHIER,PATIENTNUMBER) 
	SELECT CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->paymode'),CONCAT('$processbill->paymentreff'),CONCAT('$processbill->totalcharges'),CONCAT('$processbill->discount'),CONCAT('$processbill->vat'),CONCAT('$processbill->actualcharges'),DATE_ADD(NOW(), INTERVAL 10 HOUR),CONCAT('$dbdetails->user'),CONCAT('$processbill->clientnumber')	FROM pendingsales  WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED'   GROUP BY PATIENTNUMBER ");	
...
$connect->query(" INSERT INTO receiptsdetails(DETAILS,UNIT,PRICE,QUANTITY,TOTAL,TAXES,GROSSTOTAL,CASHIER,DATE,RECEIPTNUMBER,PATIENTNUMBER) 
	SELECT DETAILS,UNIT,PRICE,QUANTITY,TOTAL,TAXES,GROSSTOTAL,CASHIER,DATE,CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->clientnumber') FROM pendingsales WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED' ");	
...
,CONCAT('DAY'),IFNULL(DATEDIFF(DATE_ADD(NOW(), INTERVAL 10 HOUR),ADMISSIONDATE),'NEVER') AS DDYS,CONCAT((SELECT PRICE FROM  services WHERE DETAILS REGEXP 'BED CHARGES')),CONCAT('$dbdetails->user'),DATE_ADD(NOW(), INTERVAL 10 HOUR),CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->clientnumber') FROM inpatientsrecord WHERE PATIENTNUMBER= '$processbill->clientnumber' ");
...
if($processbill->discount>0)
{
	...
	VALUES('DISCOUNT','$processbill->discount','$dbdetails->user',DATE_ADD(NOW(), INTERVAL 10 HOUR),'$processbill->receiptnumber','$processbill->clientnumber') ");	
}
...
$connect ->query("DELETE  FROM pendingsales WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED' ");
...
$connect->query("INSERT INTO invoicerecords(INVOICENUMBER,PAYMODE,PAYMENTREFF,AMOUNT,DISCOUNT,TAX,TOTALCHARGES,DATE,CASHIER,CLIENTNUMBER,INSUARANCE,INSUARANCEREFF) 
	SELECT CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->paymode'),CONCAT('$processbill->paymentreff'),CONCAT('$processbill->totalcharges'),CONCAT('$processbill->discount'),CONCAT('$processbill->vat'),CONCAT('$processbill->actualcharges'),DATE_ADD(NOW(), INTERVAL 10 HOUR),CONCAT('$dbdetails->user'),CONCAT('$processbill->clientnumber'),INSUARANCE,INSUARANCENUMBER	FROM pendingsales,patientsrecord  WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED' AND pendingsales.PATIENTNUMBER=patientsrecord.ACCOUNT GROUP BY PATIENTNUMBER ");	
$processbill->receiptnumber=str_pad($processbill->receiptnumber, 5, "0", STR_PAD_LEFT);
...
SELECT (SELECT PRICE FROM services WHERE DETAILS REGEXP 'BED CHARGES')*IFNULL(DATEDIFF(CURRENT_DATE,ADMISSIONDATE),0) AS TTL,(SELECT PRICE FROM services WHERE DETAILS REGEXP 'BED CHARGES')*IFNULL(DATEDIFF(DATE_ADD(NOW(), INTERVAL 10 HOUR),ADMISSIONDATE),0) AS GROSSTTL
,CONCAT('DAY'),IFNULL(DATEDIFF(CURRENT_DATE,ADMISSIONDATE),'NEVER') AS DDYS,CONCAT((SELECT PRICE FROM  services WHERE DETAILS REGEXP 'BED CHARGES')),CONCAT('$dbdetails->user'),DATE_ADD(NOW(), INTERVAL 10 HOUR),CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->clientnumber') FROM inpatientsrecord WHERE PATIENTNUMBER= '$processbill->clientnumber' ");
$connect->query("UPDATE  inpatientsrecord SET ADMISSIONDATE=DATE_ADD(NOW(), INTERVAL 10 HOUR)  WHERE PATIENTNUMBER='$processbill->clientnumber' ");
...
header("LOCATION:poslabandimaging.php"); exit;
?>