...
function connect($host,$user,$pass,$db) {	
	...
	if (!$conn) { $this->mysqli_error = mysqli_error(); $return = false; }
...
function list_values($tablename) {
	$sql = mysqli_query("SELECT * FROM $tablename");
...
function get_table_structure($tablename) {
	...
	$sql = mysqli_query("DESCRIBE $tablename");
	$this->fields = array();
	while ($row = mysqli_fetch_array($sql)) {
		$name = $row[0];
		$type = $row[1];
