...
$supplierpayment->supplier=$connect->real_escape_string(trim(addslashes(strtoupper($_GET['supplier']))));

$x=$connect->query("SELECT ID,INSUARANCE,BOXADDRESS,PHONENUMBER,EMAIL FROM   suppliers  WHERE INSUARANCE='$supplierpayment->supplier' " );
...
<?php 
$x=$connect->query( "SELECT DISTINCT(INVOICENUMBER) AS INVOICENUMBER,INSUARANCE  FROM invoicerecords WHERE INSUARANCE='$supplierpayment->supplier' ");
while ($data = $x->fetch_object())
{
