...
#menu{ background-color: #87CEEB; height:60px;  border-radius:2%; text-align:center  }body{ font-size:100%;}.dropdown-menu{ overflow-y: scroll; height: 300%;        //  <-- Select the height of the body
   position: absolute;
}
...
$("#registry").modal();
$("#registrytable").load("unregisteredmeterregistrytable.php #accountstable");
...
function(data){
$("#content").load("message.php #content");$("#registrytable").load("unregisteredmeterregistrytable.php #accountstable");$('#prepostmessage').modal('hide'); $('#message').modal('show'); 
...
$("#registrytable").submit(function(){$('#prepostmessage').modal('show');
$.post( "delinkmeter.php",
...
<input name='' type='checkbox' id="checkall" class='form-control input-sm'></div>
 ...
 <form id="registrytable"   method="post" action="delinkmeter.php"  >
 </form>
