...
<?php 
$x=$connect->query("SELECT PATIENTNUMBER,CLIENT  FROM pendingsales,patientsrecord WHERE ACCOUNT=PATIENTNUMBER AND  STATUS='ISSUED'  AND DETAILS IN(SELECT DETAILS FROM services ) GROUP  BY   PATIENTNUMBER ");
...
<form method="post" action="processbilll4.php" id="postpayment" >
 <div id="totalbill" >
...
while ($data = $x->fetch_object())
{
 ...
 <tr class='filterdata'  style='text-align:center;' >
			   ...
			   <td   style='text-align:right;'  ><?php print number_format($data->TOTAL,2); ?></td>
						...
						<a   href="deletebilleditem2.php?deleteitemid=<?php print $data->ID; ?>"  onclick="return confirm('DELETE ?')" > <div class="fas fa-trash" style="font-size:160%;"> </div></a>
			 </td>
	 </tr>
<?php } ?> 
