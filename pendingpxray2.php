...
$number=0;
$x=$connect->query(" SELECT pendingsales.ID,pendingsales.DETAILS,pendingsales.PATIENTNUMBER,patientsrecord.CLIENT FROM pendingsales,imagingservices,patientsrecord WHERE pendingsales.DETAILS=imagingservices.DETAILS AND pendingsales.PATIENTNUMBER=patientsrecord.ACCOUNT AND pendingsales.STATUS='' AND PATIENTNUMBER='$patient->patientnumber' ORDER BY  DATE,pendingsales.ID ");
...
<a  title="<?php print $data->CLIENT;?>" data-toggle="popover" data-trigger="hover" data-content="<?php print $data->DETAILS; ?> RESULTS" data-placement="bottom" href="imagingresults.php?id=<?php print $data->ID; ?>"  onclick="return confirm('ENTER RESULTS ?');" ><i class="fa-solid fa-radiation" style="font-size:160%;"></i></a>
			 </td>
	 </tr>
<?php }	?> 
