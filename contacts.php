...
$x = $connect ->query("SELECT * FROM users  WHERE  name='$dbdetails->user' AND password='$dbdetails->password' AND  ACCESS  REGEXP  'EDIT CONTACTS' ");
...
#idnumber-list
{
 ...
 $('[data-toggle="popover"]').popover();    
   ...
   $("#updatecontactform").load("contactstable.php #contacttable");
...
$("#acrange").serialize(),
function(data){
	$("#content").load("message.php #content"); $("#updatecontactform").load("contactstable.php #contacttable"); $('#prepostmessage').modal('hide');$('#message').modal('show'); 
});  return false;
})

$("#updatecontactform").submit(function()
{
$.post( "updatecontactform.php",
$("#updatecontactform").serialize(),
...
<div class="container">
   ...
   <form method="post" action="contactsupload.php" enctype="multipart/form-data"   id="uploads">
   
     <div class="container">
  <div class="row">
  <div class="col-sm-6"><input type="file" name="file"  id="file"   class="form-control input-sm"   required="required" /></div><div class="col-sm-6"><input type="submit"  class="btn-info btn-sm"  value="UPLOAD FILE"  /><a href="#" title="INFO" data-toggle="popover" data-trigger="hover" data-content="SEARCH   ACCOUNTS" data-placement="bottom"><button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#acrange">AC RANGE</button></a>
  
 <button class="btn-info btn-sm" onclick="window.print()">PRINT</button></div></div></div>
 </form>
...
<form id="updatecontactform" method="post" action="updatecontactform.php">
</form> 
