...
$("#deletecompany").load("companyadmin.php #zones"); 
return false;});
return false;
})
...
<body>
<div class="container" >  <a href="#" title="INFO" data-toggle="popover" data-trigger="hover" data-content="Click to add new company" data-placement="bottom"><button class="btn-info btn-sm"  data-toggle="modal" data-target="#newcompany"> NEW COMPANY</button> </a>
 <input type="text" class="form-control input-sm" id="searchtext" placeholder="Type to search" autosearch="off">
    <!-- Modal -->
  </div>
  
   <form class="modal fade" id="newcompany" role="dialog" method="post" action="newcompany.php">
  <div class="modal-dialog modal-lg"  >
  <div class="modal-content"><div class="modal-header"><div class="modal-header">ENTER COMPANY NAME</div></div>
  <div class="container">
  <div class="row">

    <div class="col-sm-8" ><input type="text" class="form-control input-sm"  name="companyname" style='text-transform:uppercase' pattern ="[A-Z,a-z]+" title="ENTER TEXTS ONLY"  required="on" autocomplete="off" /></div>
...
<form id="deletecompany" method="post" action="deletecompany.php">
<div id="zones">
<h4   style="text-align:center"><strong>COMPANY ADMIN </strong></h4>
 <table    class=" table table-hover">
	  
        <!--DWLayoutTable-->
        <thead>
          <tr>
            <td  class="theader"    height="21" valign="top" >COMPANY	  </td>
		...
		if(mysqli_num_rows($x)>0)
		{
		 ...
		 while ($y=@mysqli_fetch_array($x))
		 {
		   echo"<tr class='filterdata'>
                <td>".$y['NAME']."</td>
              <td >" ; ?>
  <a   href="deletecompany.php?name=<?php print $y['NAME'];?>" class="deletestudent">
