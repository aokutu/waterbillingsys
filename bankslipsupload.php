...
$target_dir = "uploads/";
$target_file = $target_dir . basename($_FILES["bankslips"]["name"]);
$uploadOk = 1;
$FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
////////////////////////////////
move_uploaded_file($_FILES["bankslips"]["tmp_name"], $target_file);
$filename = $_FILES['bankslips']['name'];
...
if($lines >1000){header("LOCATION:accessdenied4.php");exit;}
    ...
    if (($handle = fopen($target_file, "r")) !== FALSE) {
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
          ...
          $account = substr($data[2], 0,11);
          $paymentcode=substr($data[2],0,14);
          $paymentcode=substr($paymentcode,-3);
          $position = strstr($paymentcode, '-');
if ($position == true)
{
$paymentcode=substr($paymentcode,-2);
 $x="SELECT CODE FROM  paymentcode  WHERE   CODE ='$paymentcode' LIMIT 1  ";
		$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
...
{
 $x="SELECT CODE FROM  paymentcode  WHERE   NAME ='WATER BILL' LIMIT 1  ";
		$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		...
		$paymentcode=$y['CODE'];     
          ...
          $x= "INSERT INTO BANKTRANSACTION(`TRDATE`,ACDETAILS,REFF,AMOUNT,PAYMENTCODE) VALUES ('".$date."','".$account."','".$data[3]."','".$amnt."','$paymentcode')";
          mysqli_query($connect,$x)or die(mysqli_error($connect));
...
mysqli_query($connect,$x)or die(mysqli_error($connect));     
      ...
      mysqli_query($connect,$x)or die(mysqli_error($connect));
    $x="UPDATE BANKTRANSACTION SET PAYPOINT='EQUITY' ";
    mysqli_query($connect,$x)or die(mysqli_error($connect));
...
//$x="UPDATE BANKTRANSACTION SET PAYMENTCODE = RIGHT(PAYMENTCODE,2);";mysqli_query($connect,$x)or die(mysqli_error($connect));

  $a=$connect->query("UPDATE BANKTRANSACTION SET STATUS='ACC MISSING ' WHERE ZONE IS NULL ");
   $a=$connect->query("UPDATE BANKTRANSACTION SET STATUS='PAYMENT CODE MISSING ' WHERE PAYMENTCODE NOT IN(SELECT CODE FROM  PAYMENTCODE) ");
  ...
  { 
      ...
      $connect->query("INSERT INTO $acctstable (ACCOUNT,CREDIT,DEPOSITDATE,PAYPOINT,TRANSACTION,CODE) SELECT ACDETAILS,AMOUNT,TRDATE,PAYPOINT,REFF,PAYMENTCODE FROM BANKTRANSACTION WHERE ZONE='$zone' AND STATUS IS  NULL ");
  }
  
  $a=$connect->query("UPDATE BANKTRANSACTION SET STATUS='PROCESSED' WHERE STATUS IS NULL "); 
//$connect->query("DROP TABLE  BANKTRANSACTION");    
...
$connect->query("INSERT INTO events(user,session,action,date) VALUES('$user',now(),'POSTED BANK SLIPS',now()) ");
	?>
        </tbody>
