...
#levelchart{ width:80%;   }
body{width:95%;margin-right:2%;margin-left:2%;margin-top:2%;font-size:90%; font-weight: 800; } #notice{ font-size:80%;}
...
#statement{ border-style:solid;border-radius:2%;}
  ...
  $(document).ready(function(){
   ...
   $("#statement").load("printreciept.php #billreport");
	$("#searchaccount").submit(function()
{$('#prepostmessage').modal('show');
$.post( "currentbillsession.php",
$("#searchaccount").serialize(),
function(data){$("#content").load("message.php #content"); $('#prepostmessage').modal('hide');
$('#message').modal('show'); $("#statement").load("printreciept.php #billreport");
});  return false;
})
...
<div class="container">
  ...
  <form class="modal fade" id="searchaccount" role="dialog"    action="printreciept.php" method="post"  >
  ...
  <div class="frmSearch">
RECIEPT NUMBER<a href="#" title="INFO" data-toggle="popover" data-trigger="hover" data-content="ENTER  ACCOUNT" data-placement="bottom"><input  style='text-transform:uppercase' name="account" type="text" size="15" placeholder="ENTER ACCOUNT NO."  required="on"  class="form-control input-sm"   id="search-box"  pattern="[0-9A-Za-z]{11}"  title="INVALID ENTRIES"  autocomplete="off" ></a>
