...
$x="DELETE   FROM   currentbalance  WHERE currentbalance.totalbill >0  ";
mysqli_query($connect,$x)or die(mysqli_error($connect));
...
<h3 style="text-align:center"><strong>CURRENT  ACCOUNT  STATUS  FOR ACC <?php print $account1 ;?> TO <?php print $account2;?></strong></h3>
<table class="table"  id="currentbalancetable">
	...
	$accountstable,currentbalance where    $accountstable.account=currentbalance.account order by  currentbalance.account,currentbalance.date asc ";
		$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		   ...
		   echo "<tr   class='filterdata'  >
                <td   >".$y['account']."</td>
				 <td  width='20%' >".$y['client']."</td>
				   <td style='text-align:left;' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".number_format($y['consumtion'],2)."</td>
		 ...
		 $x="SELECT COUNT(ID),SUM(CONSUMTION),SUM(BILL),SUM(BALBF),SUM(TOTALBILL) FROM  currentbalance";
		$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		 while ($y=@mysqli_fetch_array($x))
