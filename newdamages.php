...
<body>
<?php 
$x=$connect->query("SELECT *  FROM damagesregistry   WHERE ID ='$id' ");
  while ($data = $x->fetch_object())
  {  ?>
<form method="post" action="newdamages2.php" >
 ...
 <div class="container">
  <div class="row">
    ...
    <div class="col-sm-8" >
       ...
       NATIONAL ID   
	<input type="text"  value="<?php print $data->idnumber; ?>" class="form-control input-sm" style="text-transform:uppercase;"  pattern="[0-9A-Z,a-z- ]+" title="INVALID ENTRIES" placeholder="NATIONAL  ID  "  readonly name="idnumber"    required="on" /><br>
   NAME 
	<input type="text" readonly class="form-control input-sm" value="<?php print $data->name; ?>" pattern="[0-9A-Z,a-z- ]+" name="name"    required="on" /><br>
DAMAGE	DETAILS 
	<input type="text" class="form-control input-sm"  pattern="[0-9A-Z,a-z- ]+" name="details"    required="on" /><br>
AMOUNT
