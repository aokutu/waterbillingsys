...
$connect ->query("INSERT  INTO pendingsales(DETAILS,PRICE,QUANTITY,GROSSTOTAL,TOTAL,CASHIER,PATIENTNUMBER,DATE)	
SELECT DETAILS,PRICE,CONCAT('$bookprocedure->frequency'),PRICE*$bookprocedure->frequency,PRICE*$bookprocedure->frequency,CONCAT('$dbdetails->user'),CONCAT('$bookprocedure->patientnumber'),DATE_ADD(NOW(), INTERVAL 10 HOUR)  FROM imagingservices WHERE DETAILS='$bookprocedure->procedures' ");	
...
$connect ->query("INSERT  INTO pendingsales(DETAILS,PRICE,QUANTITY,GROSSTOTAL,TOTAL,CASHIER,PATIENTNUMBER,DATE)	
SELECT DETAILS,COPRATEPRICE,CONCAT('$bookprocedure->frequency'),CONCAT(COPRATEPRICE*$bookprocedure->frequency),CONCAT(COPRATEPRICE*$bookprocedure->frequency),CONCAT('$dbdetails->user'),CONCAT('$bookprocedure->patientnumber'),DATE_ADD(NOW(), INTERVAL 10 HOUR)  FROM imagingservices WHERE DETAILS='$bookprocedure->procedures' ");		
...
{
$connect ->query("INSERT  INTO pendingsales(DETAILS,PRICE,QUANTITY,GROSSTOTAL,TOTAL,CASHIER,PATIENTNUMBER,DATE)	
SELECT DETAILS,PRICE,CONCAT('$bookprocedure->frequency'),PRICE*$bookprocedure->frequency,PRICE*$bookprocedure->frequency,CONCAT('$dbdetails->user'),CONCAT('$bookprocedure->patientnumber'),DATE_ADD(NOW(), INTERVAL 10 HOUR)  FROM services WHERE DETAILS='$data' ");	
...
{
$connect ->query("INSERT  INTO pendingsales(DETAILS,PRICE,QUANTITY,GROSSTOTAL,TOTAL,CASHIER,PATIENTNUMBER,DATE)	
SELECT DETAILS,COPRATEPRICE,CONCAT('$bookprocedure->frequency'),CONCAT(COPRATEPRICE*$bookprocedure->frequency),CONCAT(COPRATEPRICE*$bookprocedure->frequency),CONCAT('$dbdetails->user'),CONCAT('$bookprocedure->patientnumber'),DATE_ADD(NOW(), INTERVAL 10 HOUR)  FROM services WHERE DETAILS='$data' ");		

}

$connect ->query("INSERT INTO events(user,session,action,date) SELECT CONCAT('$dbdetails->user'),DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 10 HOUR),CONCAT('BILLED $billing->patientnumber ',DETAILS),DATE_ADD(NOW(), INTERVAL 10 HOUR) FROM services WHERE DETAILS='$data' ");
