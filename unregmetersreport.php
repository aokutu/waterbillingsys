...
$category=$_SESSION['category'];
 $x="TRUNCATE TABLE STATEMENT";mysqli_query($connect,$x)or die(mysqli_error($connect));
	...
	$i=$y['number'];$meterstablex='meters'.$i; $accountsx='accounts'.$i;
...
<table class="table"  >
		...
		else if($category=='STATUS'){ $x="SELECT COUNT(A),B AS CAT FROM  STATEMENT GROUP BY B ORDER BY A,B";}
		else if($category=='ZONE'){ $x="SELECT COUNT(A),D AS CAT FROM  STATEMENT GROUP BY D ORDER BY A,D";}
		else if($category=='SIZE'){ $x="SELECT COUNT(A),C AS CAT FROM  STATEMENT GROUP BY C ORDER BY A,C";}
			$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
