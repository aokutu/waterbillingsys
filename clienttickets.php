...
<link rel="stylesheet"  href="stylesheets/tables.css" /><link rel="stylesheet"  href="stylesheets/dashboard.css" />
  ...
  @media print { select{display:none;} #searchtext{display:none;}} #action{display:none;}}
#levelchart{ width:80%;}
...
#searchaccounth{ border-style:solid;border-radius:2%; width:80%; margin-left:1%; margin-right:1%;}
.dropdown-menu{ overflow-y: scroll; height: 300%;        //  <-- Select the height of the body
   position: absolute;
}
body{font-size:100%;margin-left:1%;margin-right:1%;text-transform:inherit;}
...
$("#updatetickets").submit(function(){$('#prepostmessage').modal('show');

$.post( "updatetickets.php",
$("#updatetickets").serialize(),
function(data){
$("#content").load("message.php #content");$("#tickets").load("clienttickets.php #sms");$('#prepostmessage').modal('hide'); $('#message').modal('show'); 
...
$("#newticket").submit(function(){$('#prepostmessage').modal('show');
$.post( "newticket.php",
$("#newticket").serialize(),
function(data){
$("#content").load("message.php #content");$("#tickets").load("clienttickets.php #sms");$('#prepostmessage').modal('hide'); $('#message').modal('show'); 
...
<div class="container">
  ...
  <a href="#" title="INFO" data-toggle="popover" data-trigger="hover" data-content="NEW TICKETS" data-placement="bottom"><button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#newticket">NEW TICKET</button></a>
	
	<button class="btn-info btn-sm" onclick="window.print()">PRINT</button>  <br>
<img src="letterhead.png"    id="letterhead"  width="50%"  height="50%"  /> 
<h4   style="text-align:center"><strong>JOB TICKETS  AS  AT <?php print date('Y-d-m');?> </strong></h4>
<form id="updatetickets" method="post" action="updatetickets.php">
 <div class="container">
  <div class="row">
  ...
  <div class="col-sm-4"><input type="text" class="form-control input-sm"  id="officer" pattern="[A-Za-z 0-9]+"  title="ALPHANUMERIC  CHARACRERS  ALONE" name="officer"  placeholder="ENTER ASSIGNED OFFICER NAME" autosearch="off">
        ...
        <thead>
          <tr>
            <td  class="theader"   height="21" valign="top" >ACC	  </td>
			 ...
			 <td  class="theader"   height="21" valign="top" >COMPLAIN</td>
			<td  class="theader"   height="21" valign="top" >CATEGORY</td>
			 <td  class="theader"   height="21" valign="top" >ASSIGNED</td>
			  <td  class="theader"   height="21" valign="top" >COMPLAIN DATE</td>
			<td  class="theader"   height="21" valign="top" >TICKET</td>			  
			   <td  class="theader"    height="21" valign="top" >
			   <select class='form-control'   name='action' required="on"   id="selectaction" >
 <option value=''>SELECT ACTION</option>

			  <option value='POSTSMS'>POST TICKET NUMBER</option>
	...
	$x="SELECT *,DATEDIFF(CURRENT_DATE,tickets.date) FROM tickets  ORDER  BY  date ,account,contact ASC";
		$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		 ...
		 while ($y=@mysqli_fetch_array($x))
		 {
		   echo"<tr class='filterdata'>
				...
				<td  >".$y['category']."</td>
				<td  >".$y['assign']."</td>
				 <td  >".$y['date']."--DAYS--".$y['DATEDIFF(CURRENT_DATE,tickets.date)']."</td>
				  <td  >".$y['ticket']."</td>
				 <td><input name='id[]' type='checkbox' value='".$y['ticket']."'   class='form-control input-sm'> </td> 
           </tr>";
		 }
		 }
...
<button type="submit" class="btn-info btn-sm" >SUBMIT</button><button type="reset" class="btn-info btn-sm">RESET</button>
  ...
  <form class="modal fade" id="newticket" role="dialog"    action="newticket.php" method="post"  >
  <div class="modal-dialog modal-lg"  >
  <div class="modal-content"><div class="modal-header"><div class="modal-header">
  <div class="container">
  <div class="row">
  <div class="col-sm-8">  <br>
ENTER THE CUSTORMER COMPLAINTS<hr>
  <div class="frmSearch">
...
<select class='form-control'   name='category' required="on"   id="category" >
 <option value=''>SELECT CATEGORY</option>

			  <option value='BILLING'>BILLING COMPLAIN</option>
              <option value='PAYMENT'>PAYMENT COMPLAIN</option>
			  <option value='TECHNICAL'>TECHNICAL COMPLAIN</option>
			  <option value='OTHER'>OTHER COMPLAIN</option>
			   
			  </select>
			  <br>
			  <select class='form-control'   name='mode' required="on"   id="category" >
			  <option value=''>POSTING MODE</option>
              <option value='EMAIL'>EMAIL</option>
			  <option value='SMS'>TEXT MESSAGE</option>
						  </select><br>
			  <textarea  name="complain"  style='text-transform:uppercase' required="on"  class='form-control' rows="3"   placeholder="TYPE THE  COMPLAIN HERE "></textarea>
			  <br>
