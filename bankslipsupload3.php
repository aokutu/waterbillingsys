   ...
   if (($handle = fopen($target_file, "r")) !== FALSE) {
       while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
         //$date=substr($data[1], 0,10);
          $date2 = date('Y-m-d', strtotime(str_replace('/', '.', $data[0])));
           $account = substr($data[1], 31,11);
       
       $amnt=str_replace(',', '', $data[2]);
       
       $paymentcode=substr($data[1],0,30);
          $paymentcode=substr($paymentcode,-7);
          $position = strstr($paymentcode, 'CODE');
if ($position == true)
{
...
$paymentcode=substr($paymentcode,-2);
 $x="SELECT CODE FROM  paymentcode  WHERE   CODE ='$paymentcode' LIMIT 1  ";
		...
		if(mysqli_num_rows($x)>0)
		{
		 ...
		 { 
		$paymentcode=$y['CODE'];     
		 }}else {$paymentcode='NOT APPLICABLE';}
...
$paymentcode=substr($paymentcode,-2);
 $x="SELECT CODE FROM  paymentcode  WHERE   CODE ='$paymentcode' LIMIT 1  ";
		...
		$paymentcode=$y['CODE'];     
         ...
         $x= "INSERT INTO BANKTRANSACTION(`TRDATE`,ACDETAILS,REFF,AMOUNT,PAYPOINT,PAYMENTCODE) VALUES ('".$date2."','".$account."','".$data[3]."','".$amnt."','KCB','$paymentcode')";
...
mysqli_query($connect,$x)or die(mysqli_error($connect));
   ...
   $x="UPDATE BANKTRANSACTION SET AMOUNT = REPLACE(AMOUNT, ',', '')";
      mysqli_query($connect,$x)or die(mysqli_error($connect));
    $x="UPDATE BANKTRANSACTION SET PAYPOINT='KCB' ";
    mysqli_query($connect,$x)or die(mysqli_error($connect));
  ...
  $a=$connect->query("UPDATE BANKTRANSACTION SET STATUS='ACC MISSING ' WHERE PAYMENTCODE ='NOT APPLICABLE' ");
...
$connect->query("INSERT INTO events(user,session,action,date) VALUES('$user',DATE_ADD(NOW(), INTERVAL 7 HOUR),'POSTED KCB SLIPS',DATE_ADD(NOW(), INTERVAL 7 HOUR)) ");
	?>
        </tbody>
