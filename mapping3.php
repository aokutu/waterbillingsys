...
$pipecordinates=[];
print $pipecordinates;
$x="SELECT NAME,LATTITUDE,LONGITUDE FROM CHANNELS ";
	$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		
		 while ($y=@mysqli_fetch_array($x))
		{array_push($pipecordinates,"new google.maps.LatLng(".$y['LATTITUDE'].",".$y['LONGITUDE']."),"); }}
	print_r($pipecordinates);

$cordinates=json_encode($pipecordinates);
?>
...
<link rel="stylesheet"  href="stylesheets/tables.css" /><link rel="stylesheet"  href="stylesheets/dashboard.css" />
   ...
   <script type="text/javascript"> 
      var myOptions = {
         zoom: 10,
         center: new google.maps.LatLng(-2.3780361162,40.7078731543),
...
MOUSE  POINTER POSITION
  ...
  LOCK ACCOUNT
  ACCOUNT <input type="text" name="account" class="form-control input-sm" id="account">
  ...
  LAT<input type="text" name="latt4" class="form-control input-sm" id="lattitude4">LONG<input name="long4" type="text" class="form-control input-sm"  id="longitude4">
	...
	POINT 2
  LAT<input type="text"  name="latt5" class="form-control input-sm" id="lattitude5">LONG<input type="text" name="long5"  class="form-control input-sm"  id="longitude5">
 <button type="submit" class="btn-info btn-sm" >SUBMIT</button>
 </div>
