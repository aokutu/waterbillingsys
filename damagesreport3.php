...
$advance = new advance();
$advance->idnumber=$connect->real_escape_string(trim(addslashes(strtoupper($_SESSION['idnumber']))));
$advance->name=$connect->real_escape_string(trim(addslashes(strtoupper($_SESSION['name']))));
$advance->date1=$connect->real_escape_string(trim(addslashes(strtoupper($_SESSION['date1']))));
$advance->date2=$connect->real_escape_string(trim(addslashes(strtoupper($_SESSION['date2']))));
?>
...
<div id="staffs">
<h4   style="text-align:center"><strong><?php print $advance->name;  ?>  DAMAGES REPORT FROM <?php print $advance->date1; ?>  TO <?php print $advance->date2; ?> </strong></h4>
 <table    class=" table table-hover">
   ...
   $connect->query("SET @TTL=0 ");
	$x=$connect->query("SELECT *,(@TTL := AMOUNT + @TTL) AS TTLSUM  FROM damages WHERE IDNUMBER='$advance->idnumber'AND DATE >='$advance->date1' AND DATE <='$advance->date2' ");
  ...
  { 
		   ...
		   <td>".$data->transaction."</td>
              ...
              <td >&nbsp;&nbsp;&nbsp;&nbsp;" ; ?>
                 <a   href="deletedamages.php?id=<?php print $data->id;?>"  onclick="return confirm('DELETE ?')" >
DEL
                       </a>
