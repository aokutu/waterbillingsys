...
$("#newzone").submit(function(){$('#prepostmessage').modal('show');
$.post( "newzone.php",
$("#newzone").serialize(),
function(data){
$("#content").load("message.php #content");$("#deletezones").load("zoneadmin.php #zones");$('#prepostmessage').modal('hide'); $('#message').modal('show'); 
...
<body>
<div class="container" >
         <a href="#" title="INFO" data-toggle="popover" data-trigger="hover" data-content="Click to add new zone" data-placement="bottom"><button class="btn-info btn-sm"  data-toggle="modal" data-target="#newzone"> NEW ZONE</button> </a>
 ...
 <input type="text" class="form-control input-sm" id="searchtext" placeholder="Type to search" autosearch="off">
   ...
   <form class="modal fade" id="newzone" role="dialog" method="post" action="newzone.php">
  <div class="modal-dialog modal-lg"  >
  <div class="modal-content"><div class="modal-header"><div class="modal-header">ENTER ZONE DETAILS </div></div>
  <div class="container">
  <div class="row">

    <div class="col-sm-8" >ZONE NAME 
	<input type="text" class="form-control input-sm" style="text-transform:uppercase;"  pattern="[0-9A-Z,a-z- ]+" title="INVALID ENTRIES" placeholder="ENTER ZONE NAME " name="zonename"    required="on" /><br>
	ZONE NUMBER 
	<input type="number" class="form-control input-sm"  name="zonenumber"    required="on" /></div>
...
<form id="deletezones" method="post" action="deletezones.php">
<div id="zones">
<h4   style="text-align:center"><strong>ZONE ADMIN </strong></h4>
 <table    class=" table table-hover">
	  
        <!--DWLayoutTable-->
        <thead>
          <tr>
		  <td  class="theader"    height="21" valign="top" >ZONE NUMBER	  </td>
  ...
  { 
		   ...
		   <td>".$data->number."</td>
                <td>".$data->zone."</td>				
              <td > " ; ?>
                 <a   href="deletezones.php?zone=<?php print $data->number;?>"  onclick="return confirm('DELETE ?')" >
DEL
                       </a>
                       <?php               
                         print "</td>  
