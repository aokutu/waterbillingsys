...
//$("#processbill").load("pointofsale2.php #billtable");
//$('#selecteditem').focus();
...
$("#frequencyX").change(function() {
	alert();return false;
	$('#prepostmessage').modal('show');
	$.post("loadprices2.php",
$("#selectedservice,#servicepatientnumber,#frequency").serialize(),
function(data){
	$("#loadprice").load("loadprices.php #itemdetails");
	$("#loadbatchnumber").load("loadprices.php #bestbatch");
	});
...
PHAMARCUTICALS<input name="selecteditem" type="text" autofocus id="selecteditem" list="details" class="form-control input-sm" required placeholder="SEARCH DETAILS" autocomplete="off">
...
<div class="col-sm-4">
QNTITY<input type="text" required name="selectquantity" id="selectquantity" class="form-control input-sm" required placeholder="QNTY" autocomplete="off" >
...
?>
 ...
 <form id="billservice" method="post"  action="bookprocedure2.php">
 <div class="container"  >
  <div class="row">
  <div class="col-sm-4">
  <input id="servicepatientnumber" type= "hidden" name= "patientnumber" value= "<?php print $_SESSION['patientnumber'];?>" >SERVICES
...
<div id="serviceunitprice" ></div>
  </div>
    <div class="col-sm-4">FREQUENCY 
    <input name="frequency[]" type="text"  required id="frequency"  class="form-control input-sm" required placeholder="ENTER FREQUENCY" autocomplete="off">
	
 </div>
  <div class="col-sm-4"><br>
    <input name="totalservicecharges" type="text" readonly id="totalservicecharges"  class="form-control input-sm" required placeholder="TOTAL SERVICE CHARGES" autocomplete="off">
...
<td  class="theader"    height="28" valign="top"  style='text-align:right;' >STATUS</td>			
			 ...
			 <td  class="theader"    height="28" valign="top"  style='text-align:right;' >TOTAL</td>
			   <td  class="theader"  height="28" valign="top" style='text-align:center;'  > ACTION </td> </tr>
				...
				<?php
$x=$connect->query("SELECT DETAILS,PRICE,ID,STATUS,UNIT,TOTAL,TAXES,GROSSTOTAL,QUANTITY,STATUS,PATIENTNUMBER,DATE  FROM pendingsales WHERE  PATIENTNUMBER= '".$_SESSION['patientnumber']."'  ");
while ($data = $x->fetch_object())
{
	$number+=1;	?>
