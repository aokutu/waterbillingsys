...
#dashboard{
  ...
  $(document).ready(function(){
	  $("#slips").load("banking2.php #slipstable");
...
return false;
	...
	$("#searchslipcode").submit(function(){$('#prepostmessage').modal('show');
$.post( "searchbanktransaction.php",
$("#searchslipcode").serialize(),
function(data){
$("#content").load("message.php #content");$("#slips").load("banking2.php #slipstable"); $('#prepostmessage').modal('hide');$('#message').modal('show'); 

return false;});
return false;
})  

$("#processslips").submit(function(){$('#prepostmessage').modal('show');
$.post( "processslips.php",
$("#processslips").serialize(),
function(data){
$("#content").load("message.php #content");$("#slips").load("banking2.php #slipstable"); $('#prepostmessage').modal('hide');$('#message').modal('show'); 
...
<body>
  <div class="container">
  <div class="row">
  <div class="col-sm-6">  <button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#searchslipcode">SEARCH SLIPS</button> 
...
<!-- 	dashboard-->
  ...
  <form class="modal fade" id="searchslipcode" method="post" action="searchbanktransaction.php"   role="dialog"   >
  <div class="modal-dialog modal-lg"  >
  <div class="modal-content"><div class="modal-header"><div class="modal-header">
<div class="container">
  <div class="row">
   ...
   <div class="col-sm-8">
    <input type="date" class="form-control input-sm"     name="date1"     /><br>
	<input type="date" class="form-control input-sm"     name="date2"     /><br>
    <input type="text" class="form-control input-sm"  pattern="[0-9a-zA-Z()]+"  title="ENTER ANY ALPHA NUMERIC .@ PATTERN"    style='text-transform:uppercase' name="searchvalue"     placeholder="ENTER VALUE"  />
 <br>
...
<option value="uploaddate">UPLOAD DATE RANGE </option>
<option value="slipcode">SLIP CODE </option>
</select>
   <br>
   <button type="submit" class="btn-info btn-sm" >SUBMIT</button><button type="reset" class="btn-info btn-sm">RESET</button><button type="button" class="btn-info btn-sm" data-dismiss="modal" id="procedureclose">CLOSE</button>
   </div>
...
<input name='' type='checkbox' id="checkall" class='form-control input-sm'></div>
  ...
  <form  action="processslips.php" method="post" id ="processslips">
  <div id="slips" >
