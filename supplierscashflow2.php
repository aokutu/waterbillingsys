...
$cashflow->suppliercompany=$connect->real_escape_string(trim(addslashes(strtoupper($_SESSION['suppliercompany']))));
        ...
        if (confirmdelete && accessname != null && accesspass != null) {
            $.ajax({
                ...
                success: function(response) {
                    $("#patientinvoices").load("supplierscashflow2.php #patientinvoicestable", function() {
...
<div id="patientinvoicestable">
	<h3  style="text-align:center;font-weight:bold;text-decoration:underline;" ><?php 
	
	print $cashflow->suppliercompany;  ?><br>FROM <?php print $cashflow->date1; ?> TO <?php print $cashflow->date2; ?></h3>
...
$connect->query(" INSERT INTO  cashflow(COMPANY,TRANSACTION,AMOUNT,DATE) SELECT SUPPLIER,CONCAT('COMPUTE BALANCE'),SUM(PRICE) AS  AMOUNT,CONCAT('$cashflow->date1') FROM stockin WHERE SUPPLIER='$cashflow->suppliercompany' AND DATE <='$cashflow->date1'  " );
$connect->query(" INSERT INTO  cashflow(COMPANY,TRANSACTION,AMOUNT,DATE) SELECT SUPPLIER,CONCAT('COMPUTE BALANCE'),SUM(AMOUNT)*-1 AS  AMOUNT,CONCAT('$cashflow->date1')   FROM supplierpayment  WHERE SUPPLIER='$cashflow->suppliercompany' AND DATE <='$cashflow->date1' " );
...
$connect->query(" INSERT INTO  cashflow(COMPANY,TRANSACTION,TRANSACTIONREFF,AMOUNT,DATE) SELECT SUPPLIER,CONCAT('INVOICE'),INVOICENUMBER,SUM(PRICE),DATE FROM stockin WHERE SUPPLIER='$cashflow->suppliercompany' AND DATE >='$cashflow->date1' AND DATE <='$cashflow->date2' GROUP BY INVOICENUMBER " );
$connect->query(" INSERT INTO  cashflow(COMPANY,TRANSACTION,TRANSACTIONREFF,AMOUNT,DATE) SELECT SUPPLIER,PAYMODE,PAYREFFERENCE,SUM(AMOUNT)*-1,MIN(DATE) AS DATE   FROM supplierpayment  WHERE SUPPLIER='$cashflow->suppliercompany' AND DATE >='$cashflow->date1' AND DATE <='$cashflow->date2' GROUP BY PAYMODE,PAYREFFERENCE " );
...
while ($data = $x->fetch_object())
{
 ...
 <td  class="theader"    height="21" valign="top" ><?php print $data->DATE; ?></td> 
  ...
  <td  class="theader"    height="21" valign="top" ><?php print number_format($data->TTLSUM,2); ?></td> 
				            <td style='text-align:center;'  >
<?php  
if(($data->TRANSACTION=='INVOICE')  OR ($data->TRANSACTION=='OPENING BALANCE')){} else
{ ?>
