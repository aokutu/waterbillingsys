...
$x="SELECT SUM(balance) FROM  $billstable WHERE account='$account' AND date <'$date1'  ";
	 ...
	 $x="SELECT SUM(amount) FROM  $nonwaterbills WHERE account='$account' AND date <'$date1'  ";
		  	$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		 while ($y=@mysqli_fetch_array($x))
		 {$nonwaterpreviousbill=$y['SUM(amount)'];}}
	 $previousbill=$nonwaterpreviousbill+$previousbill;
		 
		  $x="SELECT SUM(credit) FROM $wateraccountstable  WHERE account='$account' AND depositdate <'$date1'    ";
		  	$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		 while ($y=@mysqli_fetch_array($x))
		 {$previousdepsit=$y['SUM(credit)'];}}
		  $previousbill= $previousbill-$previousdepsit;
...
<h4   style="text-align:center"><strong>MINI STATEMENT FOR  ACCOUNT <?php print $account;?> FROM  <?php print $date1;?>.TO<?php print $date2;?> </strong></h4>
...
<td  class="theader"  height="21" style="text-align:center;" valign="top" > TOTAL &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </td> 
<td  class="theader"   height="21"   style="text-align:center;" valign="top" > BALBF  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </td>
...
<?php
	...
	echo"<tr  class='filterdata' >
				...
				<td style='text-align:center;'   >".number_format($y['TTLSUM'],2)."</td>
           </tr>";
