...
$points=[];
	$x="SELECT LATTITUDE,LONGITUDE FROM mapping  ";
		$x=mysqli_query($connect,$x)or die(mysqli_error($connect));
		if(mysqli_num_rows($x)>0)
		{
		 while ($y=@mysqli_fetch_array($x))
		{
		$new = array_push($points, $y['LATTITUDE']);	
		}
...
<div class="container" >
  ...
  <a href="map.html" download="offlinemap.html"   title="CLICK TO" data-toggle="popover" data-trigger="hover" data-content=" DOWNLOAD MAP" data-placement="bottom"  ><input type="button"  class="btn-info btn-sm"  value="DOWNLOAD MAP"  /></a>
    <a href="map.php" target="_self"   title="CLICK TO" data-toggle="popover" data-trigger="hover" data-content=" DISPLAY MAP (INTERNET )" data-placement="bottom"  ><input type="button"  class="btn-info btn-sm"  value="DISPLAY MAP"  /></a>
