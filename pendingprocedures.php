...
$number=0;
$x=$connect->query(" SELECT pendingsales.PATIENTNUMBER,patientsrecord.CLIENT FROM pendingsales,services,patientsrecord WHERE pendingsales.DETAILS=services.DETAILS AND pendingsales.PATIENTNUMBER=patientsrecord.ACCOUNT AND pendingsales.STATUS='' GROUP BY  pendingsales.PATIENTNUMBER ORDER BY  DATE,pendingsales.ID ");
while ($data = $x->fetch_object())
{
	$number+=1;	?>
...
<a  title="<?php print $data->CLIENT;?>" data-toggle="popover" data-trigger="hover" data-content="VIEW  PENDING  PROCEDURES" data-placement="bottom" href="pendingprocedures2.php?patientnumber=<?php print $data->PATIENTNUMBER; ?>"  onclick="return confirm('VIEW PENDING PROCEDURES ?');" ><i class="fas fa-flask" style="font-size:160%;"></i></a>
			 </td>
	 </tr>
<?php }	?> 
