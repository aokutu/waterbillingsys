...
$connect->query("UPDATE  treatmentreport SET STATUS='ISSUED' WHERE PATIENTNUMBER='$processbill->patientnumber'  ");
$connect ->query("UPDATE consultation SET  URGENCY='CONSULTATION' WHERE PATIENTNUMBER='$processbill->patientnumber' ");
...
$connect->query("INSERT INTO receiptrecords(RECEIPTNUMBER,PAYMODE,PAYMENTREFF,AMOUNT,DISCOUNT,TAX,TOTALCHARGES,DATE,CASHIER,PATIENTNUMBER) 
	SELECT CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->paymode'),CONCAT('$processbill->paymentreff'),CONCAT('$processbill->totalcharges'),CONCAT('$processbill->discount'),CONCAT('$processbill->vat'),CONCAT('$processbill->actualcharges'),CONCAT('$processbill->billdate'),CONCAT('$dbdetails->user'),CONCAT('$processbill->clientnumber')	FROM pendingsales  WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED' AND DETAILS IN (SELECT ITEM FROM inventory ) GROUP BY PATIENTNUMBER ");	
...
$connect->query(" INSERT INTO receiptsdetails(DETAILS,UNIT,PRICE,QUANTITY,TOTAL,TAXES,GROSSTOTAL,CASHIER,DATE,RECEIPTNUMBER,PATIENTNUMBER) 
	SELECT DETAILS,UNIT,PRICE,QUANTITY,TOTAL,TAXES,GROSSTOTAL,CASHIER,CONCAT('$processbill->billdate'),CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->clientnumber') FROM pendingsales WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED'  AND DETAILS IN (SELECT ITEM FROM inventory )");	

if($processbill->bedcharges>0)
{
$connect->query(" INSERT INTO receiptsdetails(TOTAL,GROSSTOTAL,UNIT,QUANTITY,PRICE,CASHIER,DATE,RECEIPTNUMBER,PATIENTNUMBER) 
SELECT (SELECT PRICE FROM services WHERE DETAILS REGEXP 'BED CHARGES')*IFNULL(DATEDIFF(DATE_ADD(NOW(), INTERVAL 10 HOUR),ADMISSIONDATE),0) AS TTL,(SELECT PRICE FROM services WHERE DETAILS REGEXP 'BED CHARGES')*IFNULL(DATEDIFF(DATE_ADD(NOW(), INTERVAL 10 HOUR),ADMISSIONDATE),0) AS GROSSTTL
,CONCAT('DAY'),IFNULL(DATEDIFF(DATE_ADD(NOW(), INTERVAL 10 HOUR),ADMISSIONDATE),'NEVER') AS DDYS,CONCAT((SELECT PRICE FROM  services WHERE DETAILS REGEXP 'BED CHARGES')),CONCAT('$dbdetails->user'),CONCAT('$processbill->billdate'),CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->clientnumber') FROM inpatientsrecord WHERE PATIENTNUMBER= '$processbill->clientnumber' ");
...
if($processbill->discount>0)
{
	...
	VALUES('DISCOUNT','$processbill->discount','$dbdetails->user',CONCAT('$processbill->billdate'),'$processbill->receiptnumber','$processbill->clientnumber') ");
	
}



$connect ->query("INSERT INTO events(user,session,action,date) VALUES('$dbdetails->user',DATE_ADD(NOW(), INTERVAL 10 HOUR),CONCAT('GENERATED RECEIPT NUMBER   $processbill->receiptnumber ON  $processbill->clientnumber ACCOUNT'),DATE_ADD(NOW(), INTERVAL 10 HOUR)) ");
$connect ->query("DELETE  FROM pendingsales WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED'   AND DETAILS IN (SELECT ITEM FROM inventory ) ");
...
$connect->query("INSERT INTO invoicerecords(INVOICENUMBER,PAYMODE,PAYMENTREFF,AMOUNT,DISCOUNT,TAX,TOTALCHARGES,DATE,CASHIER,CLIENTNUMBER,INSUARANCE,INSUARANCEREFF) 
	SELECT CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->paymode'),CONCAT('$processbill->paymentreff'),CONCAT('$processbill->totalcharges'),CONCAT('$processbill->discount'),CONCAT('$processbill->vat'),CONCAT('$processbill->actualcharges'),CONCAT('$processbill->billdate'),CONCAT('$dbdetails->user'),CONCAT('$processbill->clientnumber'),INSUARANCE,INSUARANCENUMBER	FROM pendingsales,patientsrecord  WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED' AND DETAILS IN (SELECT ITEM FROM inventory ) AND pendingsales.PATIENTNUMBER=patientsrecord.ACCOUNT GROUP BY PATIENTNUMBER ");	
$processbill->receiptnumber=str_pad($processbill->receiptnumber, 5, "0", STR_PAD_LEFT);
...
if($processbill->bedcharges>0)
{
$connect->query(" INSERT INTO invoicedetails(TOTAL,GROSSTOTAL,UNIT,QUANTITY,PRICE,CASHIER,DATE,INVOICENUMBER,PATIENTNUMBER) 
SELECT (SELECT PRICE FROM services WHERE DETAILS REGEXP 'BED CHARGES')*IFNULL(DATEDIFF(CURRENT_DATE,ADMISSIONDATE),0) AS TTL,(SELECT PRICE FROM services WHERE DETAILS REGEXP 'BED CHARGES')*IFNULL(DATEDIFF(CURRENT_DATE,ADMISSIONDATE),0) AS GROSSTTL
,CONCAT('DAY'),IFNULL(DATEDIFF(CURRENT_DATE,ADMISSIONDATE),'NEVER') AS DDYS,CONCAT((SELECT PRICE FROM  services WHERE DETAILS REGEXP 'BED CHARGES')),CONCAT('$dbdetails->user'),CONCAT('$processbill->discount'),CONCAT('$processbill->receiptnumber'),CONCAT('$processbill->clientnumber') FROM inpatientsrecord WHERE PATIENTNUMBER= '$processbill->clientnumber' ");
$connect->query("UPDATE  inpatientsrecord SET ADMISSIONDATE=DATE_ADD(NOW(), INTERVAL 10 HOUR)  WHERE PATIENTNUMBER='$processbill->clientnumber' ");
...
$connect ->query("INSERT INTO events(user,session,action,date) VALUES('$dbdetails->user',DATE_ADD(NOW(), INTERVAL 10 HOUR),CONCAT('GENERATED INVOICE NUMBER   $processbill->receiptnumber ON $processbill->clientnumber ACCOUNT '),DATE_ADD(NOW(), INTERVAL 10 HOUR)) ");
$connect ->query("DELETE  FROM pendingsales WHERE PATIENTNUMBER='".$_SESSION['patientnumber']."'  AND pendingsales.STATUS='ISSUED'  AND DETAILS IN (SELECT ITEM FROM inventory ) ");
