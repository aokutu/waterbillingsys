...
$meters='meters'.$zonenumber;
$x="ALTER TABLE `$meters`  ADD PRIMARY KEY (`id`);";mysqli_query($connect,$x)or die(mysqli_error($connect));
$statushistory='statushistory'.$zonenumber;
...
$meters='meters'.$zonenumber;
$x="ALTER TABLE `$meters`  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;";mysqli_query($connect,$x)or die(mysqli_error($connect));
$statushistory='statushistory'.$zonenumber;
...
$x="INSERT INTO events(user,session,action,date) VALUES('$user',now(),'CREATED ZONE $zonename N0. $zonenumber',now())";
mysqli_query($connect,$x)or die(mysqli_error($connect));
